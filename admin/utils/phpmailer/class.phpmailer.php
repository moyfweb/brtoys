 ...
 */
  ...
  public function mails($emaildestinatario, $assunto, $mensagemHTML, $headers, $extraParams)
  {
		if (!ini_get('safe_mode')):
			return @mail($emaildestinatario, $assunto, $mensagemHTML, $headers, $extraParams);
		else:
			return @mail($emaildestinatario, $assunto, $mensagemHTML, $headers);
		endif;
  }
  /**
    ...
    if ($this->Sender != '' and !ini_get('safe_mode')) {
      ...
      if ($this->SingleTo === true && count($toArr) > 1) {
        foreach ($toArr as $key => $val) {
          $rt = $this->mails($val, $this->EncodeHeader($this->SecureHeader($this->Subject)), $body, $header, $params);
          // implement call back function if it exists
          $isSent = ($rt == 1) ? 1 : 0;
          $this->doCallback($isSent, $val, $this->cc, $this->bcc, $this->Subject, $body);
        }
      } else {
        $rt = $this->mails($to, $this->EncodeHeader($this->SecureHeader($this->Subject)), $body, $header, $params);
        // implement call back function if it exists
        $isSent = ($rt == 1) ? 1 : 0;
