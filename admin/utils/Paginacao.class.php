...
Class Paginacao
{
    ...
    */
	public function Paginacao($model,$recordByPage)
	{
		...
		$this->model->setLimit($this->recordByPage,$limit_start);
	...
	public function link($key,$value) {	
		if(count($_GET) < 1) return URL::link(null,(object)array($key=>$value));
		else return URL::link(null,(object)array($key=>$value)).'?'.http_build_query($_GET);
	}
    ...
    */
	public function linkPrev($show=true)
	{
		if($this->currentPage > 1)
			return '<li class="prev"><a href="'.$this->link('page',$this->currentPage-1).'">'.$this->msg_previousPage.'</a></li>';
		else if($show)
    ...
    */
	public function linkNext($show=true)
	{
		if($this->currentPage < $this->PagesFound )
			return '<li class="next"><a href="'.$this->link('page',$this->currentPage+1).'">'.$this->msg_nextPage.'</a></li>';
		else if($show)
	...
	public function getCurrentPages()
	{
		...
		for ($a = $begin; $a <= $end ; $a++):
			if ($a!=$this->currentPage):
				$result .= "<li class='pag'><a href='".$this->link('page',$a)."' title='".$this->msg_page.": $a'>$a</a></li>";
			else:
